...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_01.OLSR.html" style="color: blue;">
  Ordinary Least Squares Regression (`OLSR`) en R
</a>

<a href="https://dvillasanao.github.io/ML_Examples/R/01_Regression/01_01_OLSR_py.html" style="color: blue;">
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_02_Logit.html" style="color: blue;">
  Regresión Logística (`Logit`) en R
</a>
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_07_GLM.html" style="color: blue;">
  Generalized Linear Model (`GLM`) en R
</a>
...
aplica_glm <- c(
  "✅ Supervisado",
  "✅ Numérica, conteo, binaria, etc.",
  "✅ Numéricas y categóricas",
  "✅ No lineal (transformada)",
  "❌ No es requisito",
  "✅ Necesaria",
  "❌ No es requisito",
  "⚠️ Sí",
  ...
  "✅ Alta",
  "✅ Compatible",
  "❌ No cumple supuestos clave"
)

detalles_glm <- c(
  "Regresión o clasificación (dependiendo de la distribución y función de enlace)",
  "Requiere que la respuesta pertenezca a la familia exponencial (e.g., Normal, Poisson, Binomial, Gamma)",
  "Las categóricas se convierten a variables dummy o indicadoras.",
  "La función de enlace transforma la esperanza de la respuesta para ser lineal en los predictores.",
  "No asume normalidad en los errores, sino una distribución específica para la respuesta.",
  "Se asume que las observaciones son independientes.",
  "La varianza puede depender de la media a través de la función de varianza.",
  "Pueden influir en la estimación de los coeficientes, especialmente con algunas distribuciones.",
  "Puede generar inestabilidad en los coeficientes y dificultar la interpretación.",
  "Los coeficientes se interpretan en la escala de la función de enlace.",
  "Generalmente eficientes computacionalmente.",
  "Técnicas como k-fold cross-validation son aplicables.",
  "Por ejemplo, si la función de enlace o la distribución asumida no son adecuadas para los datos."
)
...
tabla_glm %>%
  gt() %>%
  tab_header(
    title = "Guía rápida para elegir GLM",
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_09_LARS.html" style="color: blue;">
  Least Angle Regression (`LARS`) en R
</a>
...
aplica_lars <- c(
  ...
  "✅ Necesaria",
  "✅ Deseable",
  "⚠️ Sí (más que OLS y Ridge)",
  "⚠️ Puede ser problemático (selecciona una entre correlacionadas)",
  "✅ Muy alta (ruta de coeficientes explícita)",
  "✅ Muy alta (especialmente para modelos esparsos)",
  "✅ Compatible",
  "❌ Datos con relaciones muy no lineales, ruido excesivo"
)

detalles_lars <- c(
  "Least Angle Regression (LARS) es un algoritmo para ajustar modelos lineales a datos de alta dimensión, especialmente cuando se espera que el modelo final sea esparso (es decir, que solo un pequeño número de características sean realmente relevantes). Es similar a LASSO, pero construye la solución paso a paso, 'moviéndose' en la dirección menos angulada de los predictores correlacionados.",
  "La variable dependiente debe ser **numérica continua**. LARS está diseñado para problemas de regresión lineal.",
  "Las variables predictoras deben ser **numéricas**. Las variables categóricas deben ser transformadas a numéricas (ej., one-hot encoding). Es **crucial escalar las características** (normalización o estandarización) antes de aplicar LARS, ya que se basa en el ángulo y la correlación entre predictores.",
  "Asume una relación **lineal** entre las variables predictoras y la variable respuesta. LARS es un método para ajustar modelos lineales esparsos, por lo que las relaciones no lineales deben ser capturadas a través de transformaciones de características si es necesario.",
  "No asume estrictamente la normalidad de los residuos para el proceso de estimación, pero los errores suelen asumirse independientes e idénticamente distribuidos (i.i.d.) para inferencia estadística. Su enfoque principal es la selección de características y el encogimiento de coeficientes.",
  "Sí, se asume que las observaciones y sus errores asociados son independientes entre sí. Esto es fundamental para la validez de los estimadores y la correcta selección de variables.",
  "Es deseable que la varianza de los errores sea constante (homoscedasticidad). LARS, al igual que OLS, se basa en la minimización de la suma de errores al cuadrado y puede verse afectado por la heterocedasticidad, aunque la regularización puede ofrecer cierta robustez.",
  "Sí, LARS es **sensible a los outliers**. Como genera una ruta de solución que se basa en la correlación entre los residuos y los predictores, los outliers que afectan la correlación pueden alterar significativamente el orden en que se seleccionan las variables y la magnitud de sus coeficientes. Es potencialmente más sensible que Ridge o incluso OLS en la fase inicial de selección.",
  "Puede ser **problemático en presencia de alta multicolinealidad** si se busca un modelo donde todas las variables correlacionadas son relevantes. Al igual que LASSO, LARS tiende a seleccionar solo una de un grupo de variables altamente correlacionadas y anular las demás en etapas tempranas. Esto es bueno para la escasez pero puede no ser ideal si todas las variables correlacionadas tienen importancia teórica o práctica.",
  "La interpretabilidad es **muy alta**. LARS genera una **ruta de solución completa** para todos los valores del parámetro de regularización, mostrando exactamente cuándo cada variable entra en el modelo y cómo cambian sus coeficientes. Esto ofrece una visión profunda del proceso de selección de características y de la importancia relativa de los predictores.",
  "La velocidad y eficiencia son **muy altas**, especialmente para modelos esparsos. Es computacionalmente tan eficiente como la regresión OLS para el mismo número de predictores, y mucho más rápido que métodos de selección de subconjuntos de fuerza bruta. Es particularmente eficiente cuando el número de características es muy grande pero se espera que solo un subconjunto pequeño sea relevante.",
  "Es **compatible y muy útil**. La validación cruzada se utiliza típicamente para seleccionar el paso óptimo en la ruta de LARS (equivalente a elegir el `lambda` en LASSO) o el número de características a incluir en el modelo final, equilibrando el ajuste y la complejidad.",
  "No funciona bien si: 1) las **relaciones subyacentes son altamente no lineales** y no pueden ser capturadas por transformaciones de características, 2) hay **ruido excesivo** en los datos que puede hacer que la ruta de selección de variables sea inestable o se incluyan variables irrelevantes, o 3) se requiere que **todos los predictores correlacionados** sean retenidos en el modelo final con coeficientes no nulos (en cuyo caso Elastic Net podría ser mejor)."
)

tabla_lars <- data.frame(Criterio = criterios_lars, Aplica = aplica_lars, Detalles = detalles_lars)

tabla_lars %>%
  gt() %>%
  tab_header(
    title = "Guía rápida para elegir Least Angle Regression (LARS)",
    subtitle = "Algoritmo de Regresión Lineal para Modelos Esparsos y Alta Dimensionalidad"
  ) %>%
  tab_footnote(footnote = "Fuente: Elaboración propia") %>%
  tab_options(
    heading.title.font.size = 14,
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_03_LOESS.html" style="color: blue;">
  Locally Estimated Scatterplot Smoothing (`LOESS`) en R
</a>
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_04_MARS.html" style="color: blue;">
  Multivariate Adaptive Regression Splines (`MARS`) en R
</a>
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_06_Polynomial-Regression.html" style="color: blue;">
...
aplica_poly_reg <- c(
  ...
  "✅ Alta",
  "✅ Compatible",
  "❌ No hay una relación polinómica clara"
)

detalles_poly_reg <- c(
  "Regresión para predecir un valor continuo.",
  "La variable dependiente debe ser continua.",
  "Funciona mejor con variables predictoras numéricas para crear términos polinómicos.",
  ...
  "Los valores atípicos pueden distorsionar significativamente la curva ajustada.",
  "Los términos polinómicos (ej., x y x^2) suelen estar altamente correlacionados, requiriendo centrado o escalado.",
  "La interpretación de los coeficientes puede ser compleja, ya que representan el cambio en la pendiente de la curva.",
  "Rápido para ajustar y predecir, similar a la regresión lineal simple.",
  "Esencial para seleccionar el grado del polinomio y evaluar la generalización del modelo.",
  "Los datos no exhiben una tendencia curvilínea, o si se elige un grado de polinomio inapropiado (muy bajo o muy alto)."
)
...
tabla_poly_reg %>%
  gt() %>%
  tab_header(
    title = "Guía rápida para elegir Regresión Polinómica",
    subtitle = "Extensión de la Regresión Lineal"
  ) %>%
  tab_footnote(footnote = "Fuente: Elaboración propia") %>%
  tab_options(
    heading.title.font.size = 14,
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_08_Quantile-Regression.html" style="color: blue;">
  Quantile Regression en R
</a>
...
aplica_quantile_reg <- c(
  ...
  "✅ Numérica continua",
  "✅ Numéricas y categóricas",
  "✅ Lineal (en cada cuantil)",
  "❌ No es requisito",
  "✅ Necesaria",
  "❌ No es requisito",
  "❌ Menos sensible",
  "⚠️ Puede afectar",
  "✅ Alta (para cada cuantil)",
  "✅ Moderada",
  "✅ Compatible",
  "❌ Se busca solo la media condicional"
)

detalles_quantile_reg <- c(
  "Regresión para modelar la relación entre predictores y un cuantil específico (ej., mediana, Q1, Q3) de la variable respuesta.",
  "La variable dependiente debe ser continua.",
  "Puede manejar ambos tipos de variables, similar a la regresión lineal.",
  "La relación es lineal en la escala de cada cuantil condicional (ej., la mediana o el percentil 90 de Y dado X).",
  "No asume normalidad en la distribución de los errores, lo que la hace robusta a distribuciones no normales.",
  "Las observaciones deben ser independientes entre sí.",
  "No asume varianza constante de los errores; de hecho, puede modelar cómo la varianza (o forma de la distribución) cambia a través de los cuantiles.",
  "Utiliza la función de pérdida de suma de valores absolutos ponderados, que es más robusta a valores atípicos que el método de mínimos cuadrados.",
  "La multicolinealidad puede generar coeficientes inestables, aunque es menos problemática que en OLS para la inferencia.",
  "Los coeficientes se interpretan como el cambio en el cuantil especificado de la variable respuesta por unidad de cambio en el predictor, manteniendo los demás constantes.",
  "Puede ser más intensiva computacionalmente que la regresión de mínimos cuadrados ordinarios (OLS) si se modelan muchos cuantiles.",
  "Aplicable para evaluar el rendimiento del modelo en la predicción de diferentes cuantiles.",
  "El objetivo es exclusivamente entender la media condicional de la respuesta, y no la distribución completa o diferentes puntos de la distribución."
)
...
tabla_quantile_reg %>%
  gt() %>%
  tab_header(
    title = "Guía rápida para elegir Regresión Cuantílica",
...
<a href="https://dvillasanao.github.io/ML_Examples/Output/Regression/01_05_SVM.html" style="color: blue;">
  Support Vector Machine (`SVM`) en R
</a>
